...
const MILESTONES: RewardTrackMilestone[] = [
  ...
  { sp: 740, id: 'sharp-nose',      icon: 'accuracy',  title: 'Sharp Nose',      description: '', type: 'REWARD TRACK', modifierKey: 'instrumentAccuracy', modifierValue: 0.01 },
...
describe('computeRewardTrackModifiers', () => {
  ...
  it('cumulative modifiers at 290 SP (end of Tier 2)', () => {
    const mods = computeRewardTrackModifiers(290, MILESTONES)
    expect(mods.movementSpeed).toBeCloseTo(0.05, 3)
    expect(mods.analysisSpeed).toBeCloseTo(0.05, 3)
    expect(mods.powerConsumption).toBeCloseTo(-0.05, 3)
