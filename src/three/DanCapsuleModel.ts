...
const MATERIAL_ROLES: Record<string, (accent: THREE.Color) => THREE.MeshStandardMaterial> = {
  Metal: (accent) => new THREE.MeshStandardMaterial({
    color: 0x3a3a3e,
    roughness: 0.55,
    metalness: 0.85,
    envMapIntensity: 0.6,
...
function loadTemplate(): Promise<THREE.Group> {
  ...
  if (!loadPromise) {
    loadPromise = new GLTFLoader()
      .loadAsync(BIOCAPSULE_GLB)
      .then((gltf) => {
        // Strip embedded lights — we use scene lighting
...
/**
 * Applies proper PBR materials to a cloned bio capsule instance.
...
function applyMaterials(root: THREE.Object3D, accent: THREE.Color): void {
  root.traverse((child) => {
    ...
    if (role && MATERIAL_ROLES[role]) {
      // Dispose the cloned GLB material before replacing
...
/**
 * Scales and positions a bio capsule instance at world coordinates.
 */
...
/**
 * Disposes all geometries and materials under a bio capsule instance.
 */
...
/**
 * Creates and places a bio capsule buildable in the scene.
 *
