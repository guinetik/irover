...
| `mars-dan-archive-v1` | JSON array of DAN prospect rows; water-confirmed entries may include `drillSiteX` / `drillSiteY` / `drillSiteZ` so the drill disc + `/dan.glb` marker can be restored after reload (see `docs/dan-drill-site-marker.md`). |
